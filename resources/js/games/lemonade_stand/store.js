  ...
  mutations: {
    doSales(state, salesData) {
      ...
      if (salesData.glasses <= 0)
        state.errors.push(
          "Bạn phải nhập một số lượng ly nước chanh > 0 để bán."
        );

      if (salesData.signs < 0)
        state.errors.push("Bạn chỉ có thể mua với một số hợp lệ");
      if (salesData.cost < salesData.costOfLemonade)
        state.errors.push(
          "Bạn không thể bán ly nước chanh với giá thấp hơn giá trị của chúng."
        );
      ...
      if (totalCost > state.assets)
        state.errors.push(
          `Chi phí của bạn (${totalCost / 100}) nhiều hơn những gì bạn có.`
        );
      ...
      */
      ...
      if (didItRain) state.message = "Trời đã mưa!";
    },
